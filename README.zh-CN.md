...
**语言怎么定？**
展示语言按系统 locale 自动判定（`lang: auto`），可手动覆盖：`~/.claude/goal-kick/venv/bin/python -m goal_poller config set lang zh`（或 `en`）。斜杠命令始终跟随你输入的语言。

**代理怎么配？**
默认直连，且自动尊重 `HTTPS_PROXY`/`HTTP_PROXY` 环境变量。需要固定代理时：`~/.claude/goal-kick/venv/bin/python -m goal_poller config set proxy http://127.0.0.1:7890`（Clash 常见端口）。
...
# 手动触发动画链路（必须指定球队，无默认值）
plugin/scripts/trigger-test.sh --team 阿根廷 --flag 🇦🇷
```
