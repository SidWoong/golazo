...
| `/golazo:follow 阿根廷和中国` | 增加关注（模糊匹配；不在 48 强的队如"中国"会被友好拒绝，同批其他球队不受影响） |
| `/golazo:unfollow 中国`   | 取消关注 |
...
**关注的球队不在世界杯怎么办？**
比如 `/golazo:follow 阿根廷和中国`：阿根廷正常关注；中国不在 48 强映射表（唯一白名单）里，会被友好拒绝且配置零写入。
