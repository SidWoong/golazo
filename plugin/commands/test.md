...
1. 若用户指定了球队，先用 `~/.claude/goal-kick/venv/bin/python -m goal_poller config search-team <名称>` 取得中文名与旗帜 emoji，组装参数；未指定则用默认（阿根廷 2-1 法国，梅西 78 分钟）。
2. 执行 `~/.claude/goal-kick/bin/trigger-test.sh`（指定球队时附加 `--team <中文名> --flag <emoji>`）。
