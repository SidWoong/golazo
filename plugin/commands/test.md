...
1. **No team given**: just run `~/.claude/goal-kick/bin/trigger-test.sh` — it re-enacts the classic scene of the 2022 Qatar final by default: Messi's 108th-minute goal, Argentina 3-2 France, in the sky-blue-and-white kit.
2. **A team was given**: first run `~/.claude/goal-kick/venv/bin/python -m goal_poller config search-team <name>` for the display name (zh or en per the user's language) and the flag emoji; then fetch the kit colors:
   `~/.claude/goal-kick/venv/bin/python -c "from goal_poller import teams; print(teams.kit_for(teams.search('<name>')[0]))"`
